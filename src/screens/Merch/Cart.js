...
const Cart = ({ cartNum, setCartNum}) => {
    ...
    const [isSolBalanceSufficient, setIsSolBalanceSufficient] = useState(true);
    const [totalSolNeeded, setTotalSolNeeded] = useState(2);  
    const currency = "SOL";
    ...
    const fetchSolBalance = async () => {
        if (publicKey) {
            try {
                ...
                setIsSolBalanceSufficient(balanceInSol >= totalSolNeeded);
            } catch (error) {
                            ...
                            </div> 
                            {
                                cartList.length > 0 &&
                                    <div className='w-full md:w-3/12 md:ml-8 px-4 flex flex-col mb-8'>
                                        ...
                                        <Button onClick={handleBuyAll} className={classes.button} variant="outlined" disabled={!isSolBalanceSufficient}>Buy All</Button>
                                    </div>
                            }
                        </div>
